...
if [ ! -f ~/.ekgnv ]; then \
 echo -n "Brak pliku z konfiguracj±. Tworze .ekgnv "
  touch ~/.ekgnv
  echo "export WGET=`which wget`
export TMP=/tmp
export EKGWWW=http://dev.null.pl/ekg/
export EKGCONF=\"--prefix=/usr --with-shared --with-ioctl\"
export LASTEKG=00000000" >> ~/.ekgnv
 echo " Gotowe!"
 echo
fi
...
function get_new {
 check_new
  if [ ! -z "$NEW" ]; then \
   ...
   wget -q -P $TMP $EKGWWW/$LASTES.tar.gz
 ...
 # echo "export LASTEKG=$LASTES" >> ~/.ekgtmp
   mv -f ~/.ekgtmp ~/.ekgnv
   echo "Gotowe.
Plik znajduje siê w $TMP/$LASTES.tar.gz"
  else 
   echo 
  fi
}
