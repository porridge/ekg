	...
	for (ent = buf->last_ent_start; ent; ent = ent->next) {
		if (ent->mark_start != -1) {
			...
			if (ent->mark_start == 0 && xtext->mark_stamp) {
				int stamp_size =
					strlen(timestamp_time("%H:%M:%S", ent->fstr->ts));
				len += stamp_size;
			}
		...
		if (ent->mark_start != -1) {
			...
			if (ent->mark_end - ent->mark_start > 0) {
				...
				if (ent->mark_start == 0 && xtext->mark_stamp) {
					...
					int stamp_size = strlen(time_str);
...
gtk_xtext_render_str(GtkXText * xtext, int y, textentry * ent,
		     unsigned char *str, unsigned char *attr, int len, int win_width, int indent,
...
#warning "Verify, ekg2->ekg fstring_t"
...
void gtk_xtext_append_fstring(xtext_buffer *buf, fstring_t fstr)
{
	...
	ent->fstr->attr  = xrealloc(ent->fstr->attr, sizeof(char) * (len+1));
		memmove(ent->fstr->attr+1, ent->fstr->attr, len*sizeof(char));
		ent->fstr->attr[0] = 128;
