 ...
 */
static void cv_tabs_sizerequest(GtkWidget *viewport, GtkRequisition * requisition, chanview * cv)
{
	if (!cv->vertical)
		requisition->width = 1;
	else
...
static void cv_tabs_sizealloc(GtkWidget *widget, GtkAllocation * allocation, chanview * cv)
{
	GtkAdjustment *adj;
	GtkWidget *inner;
	gint viewport_size;
...
static gint
tab_search_offset(GtkWidget *inner, gint start_offset, gboolean forward, gboolean vertical)
{
	GList *boxes;
	GList *tabs;
	GtkWidget *box;
...
static void tab_scroll_left_up_clicked(GtkWidget *widget, chanview * cv)
{
	GtkAdjustment *adj;
	gint viewport_size;
	gfloat new_value;
...
static void tab_scroll_right_down_clicked(GtkWidget *widget, chanview * cv)
{
	GtkAdjustment *adj;
	gint viewport_size;
	gfloat new_value;
...
static gboolean tab_scroll_cb(GtkWidget *widget, GdkEventScroll * event, gpointer cv)
{
...
static void cv_tabs_xclick_cb(GtkWidget *button, chanview * cv)
{
...
static GtkWidget *make_sbutton(GtkArrowType type, void *click_cb, void *userdata)
{
	...
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(click_cb), userdata);
	g_signal_connect(G_OBJECT(button), "scroll_event", G_CALLBACK(tab_scroll_cb), userdata);
...
static void cv_tabs_init(chanview * cv)
{
	...
	g_signal_connect(G_OBJECT(outer), "size_allocate", G_CALLBACK(cv_tabs_sizealloc), cv);
/*	gtk_container_set_border_width (GTK_CONTAINER (outer), 2);*/
	gtk_widget_show(outer);

	viewport = gtk_viewport_new(0, 0);
	gtk_viewport_set_shadow_type(GTK_VIEWPORT(viewport), GTK_SHADOW_NONE);
	g_signal_connect(G_OBJECT(viewport), "size_request", G_CALLBACK(cv_tabs_sizerequest), cv);
	g_signal_connect(G_OBJECT(viewport), "scroll_event", G_CALLBACK(tab_scroll_cb), cv);
	...
	button = gtkutil_button(outer, GTK_STOCK_CLOSE, NULL, cv_tabs_xclick_cb, cv, 0);
	gtk_button_set_relief(GTK_BUTTON(button), GTK_RELIEF_NONE);
...
static void cv_tabs_postinit(chanview * cv)
{
}

static void tab_add_sorted(chanview * cv, GtkWidget *box, GtkWidget *tab, chan * ch)
{
	GList *list;
	GtkBoxChild *child;
	int i = 0;
...
static void cv_tabs_prune(chanview * cv)
{
...
static void tab_add_real(chanview * cv, GtkWidget *tab, chan * ch)
{
	GList *boxes;
...
static gboolean tab_ignore_cb(GtkWidget *widget, GdkEventCrossing * event, gpointer user_data)
{
	return TRUE;
}
...
static void tab_pressed_cb(GtkToggleButton * tab, chan * ch)
{
	chan *old_tab;
...
static void tab_toggled_cb(GtkToggleButton * tab, chan * ch)
{
	if (ignore_toggle)
		return;
...
static gboolean tab_click_cb(GtkWidget *wid, GdkEventButton * event, chan * ch)
{
	return ch->cv->cb_contextmenu(ch->cv, ch, ch->tag, ch->userdata, event);
}

static void *cv_tabs_add(chanview * cv, chan * ch, char *name, GtkTreeIter * parent)
{
	GtkWidget *but;
	...
	g_signal_connect(G_OBJECT(but), "button_press_event", G_CALLBACK(tab_click_cb), ch);
	/* avoid prelights */
	g_signal_connect(G_OBJECT(but), "enter_notify_event", G_CALLBACK(tab_ignore_cb), NULL);
	g_signal_connect(G_OBJECT(but), "leave_notify_event", G_CALLBACK(tab_ignore_cb), NULL);
	g_signal_connect(G_OBJECT(but), "pressed", G_CALLBACK(tab_pressed_cb), ch);
	/* for keyboard */
	g_signal_connect(G_OBJECT(but), "toggled", G_CALLBACK(tab_toggled_cb), ch);
...
static int tab_check_focus_cb(GtkWidget *tab, int num, int unused)
{
...
static int tab_group_get_cur_page(chanview * cv)
{
	return tab_group_for_each_tab(cv, tab_check_focus_cb, 0);
}

static void cv_tabs_focus(chan * ch)
{
	if (ch->impl)
...
static int tab_focus_num_cb(GtkWidget *tab, int num, int want)
{
	if (num == want) {
...
static void cv_tabs_change_orientation(chanview * cv)
{
	/* cleanup the old one */
	if (cv->func_cleanup)
		cv->func_cleanup(cv);
...
static void cv_tabs_move_focus(chanview * cv, gboolean relative, int num)
{
	int i, max;
...
static void cv_tabs_remove(chan * ch)
{
...
static void cv_tabs_move(chan * ch, int delta)
{
	int i, pos = 0;
	GList *list;
...
static void cv_tabs_cleanup(chanview * cv)
{
	if (cv->box)
		gtk_widget_destroy(((tabview *) cv)->outer);
}

static void cv_tabs_set_color(chan * ch, PangoAttrList * list)
{
	gtk_label_set_attributes(GTK_LABEL(GTK_BIN(ch->impl)->child), list);
}

static void cv_tabs_rename(chan * ch, char *name)
{
	PangoAttrList *attr;
...
static gboolean cv_tabs_is_collapsed(chan * ch)
{
	return FALSE;
}

static chan *cv_tabs_get_parent(chan * ch)
{
	return NULL;
}
