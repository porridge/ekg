...
void theme_init()
{
	...
	format_add("statusbar", " %c(%w%{time}%c)%w %c(%w%{?!nick uin}%{nick}%c/%{?away %w}%{?avail %W}%{?invisible %K}%{?notavail %k}%{uin}%c) (%wwin%c/%w%{window}%{?query %c:%W}%{query}%c)%w%{?activity  %c(%wact%c/%w}%{activity}%{?activity %c)%w}%{?mail  %c(%ymail%c)}%{?more  %c(%Gmore%c)}%{?debug  %c(%Cdebug%c)}", 1);
