...
#include "ui-gtk-chanview-tabs.c"


/* ======= TREE ======= */

#include "ui-gtk-chanview-tree.c"
...
static chan *
chanview_add_real (chanview *cv, char *name, void *userdata,
...
chan *
chanview_add (chanview *cv, char *name, void *userdata, gboolean allow_closure, int tag, GdkPixbuf *icon)
{
	char *new_name;
	chan *ret;
	...
	ret = chanview_add_real (cv, new_name, userdata, allow_closure, tag, icon, NULL, NULL);
...
chan_emancipate_children (chan *ch)
{
	...
	while (gtk_tree_model_iter_children (GTK_TREE_MODEL (ch->cv->store), &childiter, &ch->iter))
	{
		...
		ch->cv->size--;
		chanview_add_real (childch->cv, name, childch->userdata, childch->allow_closure, childch->tag, childch->icon, childch, ch);
		if (attr)
		{
...
chan_remove (chan *ch, gboolean force)
{
	...
	if (ui_quit)	/* avoid lots of looping on exit */
		return TRUE;
